...
class YouTubeFactChecker:
    ...
    def fetch_transcript(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Could not retrieve transcript. Video may not have captions. Error: {e}"
                )
    ...
    def identify_claims(self, max_claims: int = 10) -> List[Dict[str, Any]]:
        ...
        if self.llm_provider == "openai":
            try:
                ...
                print(
                    f"Error parsing OpenAI response: {e}. Falling back to default format."
                )
            except Exception as e:
            ...
            try:
                ...
                print(
                    f"Error parsing Gemini response: {e}. Falling back to default format."
                )
            except Exception as e:
        ...
        if self.llm_provider == "openai":
            ...
            try:
                ...
                if hasattr(gemini_response, "text") and gemini_response.text:
                    ...
                    print(
                        "Error: Gemini verification response does not have text content"
                    )
    ...
    def analyze_video(self) -> Dict[str, Any]:
        ...
        if not self.video_info:
            raise ValueError(
                "Video information not available. Call fetch_video_info() first."
            )

        # Safely extract video metadata
        video_title = self.video_info.get("snippet", {}).get("title", "Unknown Title")
        channel_title = self.video_info.get("snippet", {}).get(
            "channelTitle", "Unknown Channel"
        )
        publish_date = self.video_info.get("snippet", {}).get(
    ...
    def _generate_summary(
        ...
        if not self.video_info:
            ...
            video_title = self.video_info.get("snippet", {}).get(
                "title", "Unknown Title"
            )
            channel_title = self.video_info.get("snippet", {}).get(
                "channelTitle", "Unknown Channel"
...
def main():
    ...
    try:
        ...
        print(
            "\nRELIABILITY SCORE:",
            f"{analysis['summary']['reliability_score']:.2f}/1.00",
        )
        print("\nCLAIMS ANALYZED:")
        ...
        for i, claim in enumerate(analysis["verified_claims"], 1):
            try:
                ...
                print(
                    f"   EXPLANATION: {textwrap.fill(claim.get('explanation', 'No explanation available'), width=80, initial_indent='   ', subsequent_indent='   ')}"
                )
            except Exception as e:
